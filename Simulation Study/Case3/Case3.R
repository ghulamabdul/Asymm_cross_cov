...
########################################################
setwd("D:/Projects 3/Revision codes - Simulation/Case3")
dir()
##### Loading libraries ######
library(fields)
library(MASS)
...
nu2<-2
nu3<-3
########## Setting marginal standard deviations ########
...
C13<-rhomat[1,3]*sigma1*sigma3*my.matern(h=shift.dist13,a=a13,sigma = 1,nu=nu13)

tempgrid3<-cbind(rep(a231,times=n^2),rep(a232,times=n^2))

shift23grid<-grid-tempgrid3
shift.dist23<-rdist(grid,shift23grid)
C23<-rhomat[2,3]*sigma2*sigma3*my.matern(h=shift.dist23,a=a23,sigma = 1,nu=nu23)
...
######## intitalizing value #########
init.my.pars.mat.cross<-c(0.5,0.3,0.3,-0.15,-0.14,-0.14,-0.138,-0.151,-0.15)
...
######## intitalizing value #########
init.my.sep.mat.cross<-c(0.5,0.3,0.3,-0.15,-0.14,-0.15,-0.139,-0.15,-0.14)
...
init.liz.pars.mat.cross<-c(0.5,0.3,0.3,-0.15,-0.15,-0.15,-0.15)
...
######## intitalizing value #########
init.liz.sep.mat.cross<-c(0.5,0.3,0.3,-0.15,-0.15,-0.15,-0.15) 
...
sigma3=1
### C12 ###
##### In our simulations above the asymmetry vectors are , the cross-covariances are actually C_ij(h+a_ij), to make it consistent with the papers notation of C_ij(h-a_ij) we put a_ij=-aij 
c12.image<-asym.cross(rho12,sigma1,sigma2,nu12,a=a1,as11=a121,a122,hlag1=h1lags,hlag2=h2lags)
c13.image<-asym.cross(rho13,sigma1,sigma3,nu13,a=a1,as11=a131,a132,hlag1=h1lags,hlag2=h2lags)
c23.image<-asym.cross(rho23,sigma2,sigma2,nu12,a=a1,as11=a231,a232,hlag1=h1lags,hlag2=h2lags)
