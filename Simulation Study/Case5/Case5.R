...
########################################################
#setwd("D:/Projects 3/Revision codes - Simulation/Case6")
setwd("/ibex/scratch/qadirga/Project_3/Revision/Case_62")
dir()
##### Loading libraries ######
library(fields)
library(MASS)
...
#beta<-rho_par(c(0.7,0.52,0.50)) 

#beta


### For d=2 ### 
rho12<-1*(sqrt(gamma(nu1+1))*sqrt(gamma(nu2+1))*gamma((nu1+nu2)/2))/(sqrt(gamma(nu1))*sqrt(gamma(nu2))*gamma((nu1+nu2)/2+1))
rho13<-1*(sqrt(gamma(nu1+1))*sqrt(gamma(nu3+1))*gamma((nu1+nu3)/2))/(sqrt(gamma(nu1))*sqrt(gamma(nu3))*gamma((nu1+nu3)/2+1))
rho23<-1*(sqrt(gamma(nu2+1))*sqrt(gamma(nu3+1))*gamma((nu2+nu3)/2))/(sqrt(gamma(nu2))*sqrt(gamma(nu3))*gamma((nu2+nu3)/2+1))
...
tv<-(1^2+1^2+1^2-1)<=(-2*abs(1)*abs(1)*abs(1))
tv
...
################################################
#### Computing Li-Zhangs cross-covariances #####
################################################
# C2 is the full covariance matrix
###### Asymmetry parameters 0 #####
as11=0
as12=0
as21=0.4
as22=0.6
...
######## intitalizing value #########
init.my.pars.mat.cross<-c(0.35,0.60,0.57,0.39,0.62,0.31,0.43,-0.12,-0.22)
...
######## intitalizing value #########
init.my.sep.mat.cross<-c(0.35,0.60,0.57,0.39,0.62,0.31,0.43,-0.12,-0.22)
...
init.liz.pars.mat.cross<-c(1,7,5,0.39,0.62,0.31,0.43)
...
######## intitalizing value #########
init.liz.sep.mat.cross<-c(1,7,5,0.39,0.62,0.31,0.43) 
...
init.sym.pars.mat.cross<-c(1,7,5)
...
######## intitalizing value #########
init.sym.sep.mat.cross<-c(1,7,5) 
...
image.plot(hlag1,hlag2,cross.cor,col = viridis(n=100),xlab=expression('h'[1]),ylab=expression('h'[2]),zlim=c(0,max(cross.cor)),las=1,xaxt="n",yaxt="n",legend.width = 0.5)
...
### C12 ###
par(mfrow=c(3,1))
c12.image<-asym.cross(rho12,sigma1,sigma2,nu12,a=a1,as11=-(as11-as21),as12 =-(as12-as22) ,hlag1=h1lags,hlag2=h2lags)
c13.image<-asym.cross(rho13,sigma1,sigma3,nu13,a=a1,as11=-(as11-as31),as12 =-(as12-as32),hlag1=h1lags,hlag2=h2lags)
c23.image<-asym.cross(rho23,sigma2,sigma2,nu12,a=a1,as11=-(as21-as31),as12 =-(as22-as32),hlag1=h1lags,hlag2=h2lags)
